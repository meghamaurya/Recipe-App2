...
function Page2() {
    ...
    const [id, setId] = useState('511728');

    useEffect(() => {
        fetch(`https://api.spoonacular.com/recipes/${id}/information?apiKey=${process.env.apiKey}&includeNutrition=true`)
            .then(res => res.json())
            .then((res) => {
                setData(res);
    ...
    return !loading ? <View style={s`w-full h-full bg-white flex flex-col box-border`}>
        ...
        <View style={s`bg-white box-border w-full bg-white relative`}>
            <View style={[s`p-4 absolute box-border w-full -top-10 items-center flex flex-col bg-white shadow-2xl shadow-black`, styles.detailCard]}>
                ...
                {data ? <View style={[s`w-full flex flex-row justify-between px-1 py-8`, styles.card]}>
